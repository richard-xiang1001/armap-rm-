...
def _variant_grid() -> List[Dict]:
    variants: List[Dict] = []
    idx = 0
    for neg_type in ("truncate_last_k", "swap_step", "replay_corrupt"):
        for refine_mode in ("off", "rule"):
            for input_format in ("flat", "stepwise"):
                for pooling in ("last", "last_k_step"):
                    variants.append(
                        {
                            "name": f"{idx:02d}_{neg_type}_{refine_mode}_{input_format}_{pooling}",
                            "neg_type": neg_type,
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Run v0.4.x ablations (neg/refine/format/pooling).")
    ap.add_argument("--raw_path", type=str, required=True)
    ap.add_argument("--adapter", type=str, default="generic_jsonl", choices=["generic_jsonl", "armap_style", "webshop_like", "real_logs_v1"])
    ap.add_argument("--env_judge", type=str, default="arithmetic", choices=["arithmetic", "real_logs"])
    ap.add_argument("--work_dir", type=str, default="data/ablation/v04x")
    ap.add_argument("--results_dir", type=str, default="results/ablation/v04x")
    ...
    ap.add_argument("--bucket_sampling", type=str, default="true", choices=["true", "false"])
    args = ap.parse_args()
    ...
    for cfg in _variant_grid():
        ...
        pairs_all = run_work / "pairs.all.jsonl"
        pairs_unsanitized = run_work / "pairs.trainable.unsanitized.jsonl"
        pairs_sanitized = run_work / "pairs.sanitized.jsonl"
        train_path = run_work / "train.jsonl"
        valid_path = run_work / "valid.jsonl"
        mismatch_path = run_results / "mismatches.jsonl"

        use_refined = cfg["refine_mode"] != "off"
            ...
            rows.append(
                {
                    ...
                    "judge_unknown_rate": float(lint.get("judge_unknown_rate", 0.0)),
                    "train_pair_accuracy": float(train_metrics["pair_accuracy"]),
                    "n_valid": int(eval_metrics["n"]),
                    "mismatches_path": str(mismatch_path),
                }
            )
    ...
    out_csv = results_root / "v04x_ablation.csv"
    out_md = results_root / "v04x_ablation.md"
    ...
    md_lines = [
        "# v0.4.x Ablation Results",
        "",
        "| run | status | neg_type | refine | format | pooling | pair_accuracy | gap_p50 | trunc_risk_last_k | replay_ok_rate | judge_unknown_rate |",
        "|---|---|---|---|---|---|---:|---:|---:|---:|---:|",
    ]
    for row in rows:
        md_lines.append(
            "| {run} | {status} | {neg_type} | {refine_mode} | {input_format} | {pooling} | {pair_accuracy} | {gap_p50} | {truncation_risk_last_k_steps} | {neg_replay_success_rate} | {judge_unknown_rate} |".format(
                run=row.get("run", ""),
                ...
                judge_unknown_rate=row.get("judge_unknown_rate", ""),
            )
        )

    out_md.write_text("\n".join(md_lines) + "\n", encoding="utf-8")
    print(json.dumps({"rows": len(rows), "csv": str(out_csv), "md": str(out_md)}, ensure_ascii=False))
